...
## 2026-07-29 · 首页推荐图文卡片改版

- 首页推荐改为参考图中的独立首页：顶部保留搜索和离线数据库状态，中间展示“今日推荐”图文卡片，下方展示“快速开始”食材选择入口
- “今日推荐”只从带图片的菜谱中选取，优先使用 `xiachufang-` 来源的下厨房菜谱；当前可用图文菜谱 315 道，其中下厨房图文菜谱 131 道
- 首页默认展示 5 道有图菜谱卡片，包含菜名、难度、烹饪时间和评分样式；点击卡片会进入原有菜谱浏览/详情界面
- “快速开始”按钮会进入菜谱分类页并打开食材选择器，保留原有基于食材推荐菜谱的流程
- 原有菜谱列表和详情页继续在“菜谱分类 / 收藏菜谱 / 最近浏览”中使用，并保留桌面端左右区域独立滚动
- 点列表页“推荐菜谱”时仍只显示匹配度 60% 以上结果
- 验证：`pnpm build` 通过

## 2026-07-30 · 新增全部菜谱平铺界面

- 新增侧边栏入口“全部菜谱”，不替换原有“菜谱分类 / 详情”浏览流程
- “全部菜谱”页面按参考图样式展示：顶部标题、搜索框、离线数据库状态，大白色面板内平铺所有菜谱卡片
- 页面支持分类页签：`全部`、`家常菜`、`快手菜`、`汤羹`、`素食`、`主食`
- `快手菜` 按 20 分钟内筛选；`素食` 按排除肉类/水产食材做本地判断
- 菜谱卡片展示图片、菜名、难度、烹饪时间和评分样式；没有图片的菜谱使用“待上传图片”占位卡，确保所有菜谱都能被平铺展示
- 点击任意卡片会进入原有菜谱浏览/详情界面查看完整食材和步骤
- 验证：`pnpm build` 通过
