...
导入脚本会复制可识别的菜谱图片到 `public/howtocook`，没有图片的菜谱会在界面显示“待上传图片”。
...
- `ingredients`：网站支持的食材名称、类型和 emoji
- `meta`：本地种子数据版本

无原始图片的菜谱会显示“待上传图片”，不会使用其他菜谱的图片冒充。可以在菜谱详情中上传/替换图片，也可以在“上传菜谱”页面按模板导入新菜谱。如果模板里包含食材库不存在的食材，应用会先弹窗确认是否添加该食材。
