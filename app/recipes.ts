...
const seasoningPattern =
  /盐|糖|油|醋|酱|生抽|老抽|蚝油|料酒|鸡精|味精|胡椒|花椒|八角|桂皮|香叶|淀粉|孜然|辣椒粉|香油|耗油|酒|茶|咖啡|奶油|黄油|蜂蜜|salt|sugar|oil|vinegar|sauce|soy|pepper|spice|butter|honey|cream|wine|stock|broth/i;
...
export function categoryForIngredient(
  ...
  if (/虾|鱼|蟹|贝|蛏|蛤|蚝|鲍|鱿|章鱼|海参|海带|紫菜|鳗|鳕|鲈|鲤|鲫|鳊|黄鱼|罗氏|fish|shrimp|prawn|crab|clam|oyster|salmon|tuna|cod|seafood/i.test(name))
    return "海鲜";
  if (/猪|牛|羊|鸡|鸭|鹅|蛋|肉|排骨|里脊|五花|火腿|培根|香肠|腊肠|鸡翅|鸡腿|鸭血|午餐肉|chicken|beef|pork|lamb|turkey|duck|egg|bacon|sausage|ham|meat/i.test(name))
    return "肉类";
  if (/米|饭|面|粉|粉丝|粉条|馒头|包子|饺|馄饨|饼|粥|燕麦|荞麦|意面|通心粉|年糕|吐司|面包|薯条|rice|noodle|pasta|spaghetti|bread|toast|flour|oat|cereal/i.test(name))
    return "主食";
  if (/菜|萝卜|白菜|生菜|菠菜|芹菜|青椒|辣椒|番茄|土豆|茄子|黄瓜|西葫芦|洋葱|葱|姜|蒜|蘑菇|香菇|豆腐|豆芽|玉米|莲藕|冬瓜|南瓜|莴笋|韭菜|西兰花|花菜|包菜|娃娃菜|油麦|香菜|薄荷|果|梨|橙|柠檬|苹果|香蕉|草莓|百香果|tomato|potato|onion|garlic|ginger|carrot|pepper|mushroom|corn|cabbage|lettuce|spinach|broccoli|pea|bean|apple|lemon|fruit|vegetable/i.test(name))
    return "蔬菜";
  return "调料";
}
